...
def angles_plot(save, angles_p, angles_y):
    plot_setup()
    save_path = os.path.join("Evaluation", "Runs", save)
    figsize = compute_figsize(426, 500)
    df = pd.read_csv(os.path.join(save_path, "age_evaluation.csv"))
    df_mean = df.groupby(['angle_y', 'angle_p']).mean().reset_index()
    plt.figure(figsize=compute_figsize(300, 230), dpi=300)
    x = df_mean.angle_y.to_numpy()
    y = df_mean.angle_p.to_numpy()
    # X, Y = np.meshgrid(x, y)
    z = df_mean.magface.to_numpy()
