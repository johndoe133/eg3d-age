...
def plot_fancy_age(network_folder, save_name, compare_baseline=True):
    ...
    df2['target'] = df2.name.apply(lambda x: float(x.split("_")[-1].split("-")[0].replace(",",".")))
    ...
    plt.scatter(df_fancy_group['target'], df_fancy_group['error'], s=10, zorder=20)
    plt.plot(df_fancy_group['target'], df_fancy_group['error'])
    plt.fill_between(df_fancy_std['target'], df_fancy_group['error'] + df_fancy_std['error'], df_fancy_group['error'] - df_fancy_std['error'], facecolor='C0', alpha=0.3, label='std')
    plt.gca().set_box_aspect(1)
    plt.xticks(np.arange(0,90,10))
    plt.title(r"$\texttt{cognitec}$")
    # FPAge
    ...
    plt.fill_between(df['target_age'], df['age_difference_fpage'] + df['std'], df['age_difference_fpage'] - df['std'], facecolor='C1', alpha=0.3, label='std')
    lower, upper = plt.ylim()
    plt.ylim(0, upper)
    plt.xlabel('Target age')
    plt.gca().set_box_aspect(1) 
    plt.xticks(np.arange(0,90,10))
    plt.title(r"$\texttt{FPAge}$")
    plt.tight_layout()
